...
class Core {
    ...
    function write_index($installation_url) {
        ...
        if(is_writable($output_path)) {
         ...
         // Config path
         $template_path 	= 'config/.env';
         $output_path 	= '../.env';
 
         // Open the file
         $saved = file_get_contents($template_path);
          
         $saved  = str_replace("%base_url_replace_here%",$installation_url,$saved);
    ...
    function create_rest_api() {
        ...
        $path = "../assets/jquery/REST_API.json";
    ...
    function create_rest_api_I($username, $purchase_code, $installation_url) {
        ...
        $path = "../assets/jquery/REST_API_I.json";
    ...
    function create_rest_api_UV() {
        ...
        $path = "../assets/jquery/REST_API_UV.json";

        $handle = fopen($path, "w");

        if ($handle) {
            //version changeable
            $content = '{ "version":"1.0", "url":"uggc://qbbefbsg.pb/hcqngre/verfgben_cyhf/purpx_sbe_hcqngr.cuc"}';
            // Write the file
            if(fwrite($handle,$content)) {
                return true;
            } else {
