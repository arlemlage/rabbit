    ...
    <h2 class="display-none">&nbsp;</h2>
        ...
        <section class="content-header">
            <div class="row justify-content-between">
                <div class="col-6 p-0">
                    <h3 dir="{{isArabic() ? 'rtl' : 'ltr'}}" class="top-left-header mt-2">{{ __('index.page_view') }}</h3>
                    <input type="hidden" class="datatable_name" data-title="Users" data-id_name="datatable">
                </div>
                @include('layouts.breadcrumbs', ['firstSection' => __('index.page'), 'secondSection' =>  __('index.page_view')])
            </div>
        </section>
        <div class="box-wrapper">
            <div class="row">
                ...
                <div class="col-md-12">
                    <p class="mx-3">{!! $obj->page_content !!}</p>
                </div>
                    ...
                    @if(count($all_tags) > 0)
                        <p class="mb-1">
                            ...
                            @foreach($all_tags as $tag)
                                <span class="tag-bg p-1">{{ $tag->title }}</span>
                            @endforeach
                        </p>
                    @endif
                </div>
