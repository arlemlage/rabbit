        ...
        <div class="row">
                        ...
                        <div class="media-size-email">
                            @if (Auth::user()->image != null and file_exists(Auth::user()->image))
                                <img width="52" height="52" class="me-3 rounded-circle"
                                    src="{{ asset(Auth::user()->image) }}" alt="Image">
                            @else
                                <img width="52" height="52" class="me-3 rounded-circle"
                                    src="{{ asset('assets/images/avatar.png') }}" alt="Image">
                            @endif
                        </div>
            ...
            <div class="col-md-8 col-lg-9 mt-2">
                <div class="table-card">
                    <h2 class="top-left-header">@lang('index.open_tickets')</h2>
                    <div class="card-body table-responsive profile_min_height">
                        <input type="hidden" class="datatable_name" data-title="@lang('index.ticket')"
                        ...
                        <table id="datatable" class="table table-responsive">
                            ...
                            @php
                                $results = \App\Model\Ticket::with('getProductCategory', 'getCustomer')
                                    ->ticketCondition()
                                    ->type()
                                    ->live()
                                    ->where('status', 1)
                            ...
                            <tbody>
                                @foreach ($results as $data)
                                    <tr>
                                        ...
                                        <td title="{{ $data->title ?? '' }}">
                                            <a title="{{ $data->title }}"
                                                class="gray-color text-decoration-none {{ needAction($data->id) ? 'text-bold' : '' }}"
                                                href="{{ url('ticket/' . encrypt_decrypt($data->id, 'encrypt')) }}">
                                        ...
                                        <td>
                                            <a href="{{ url('ticket/' . encrypt_decrypt($data->id, 'encrypt')) }}"
                                                class="btn user-home-page-btn mt-2 ms-md-0 mt-md-2 mt-sm-0 ms-sm-2 mt-lg-0 ms-lg-2">
                                                <i class="fa fa-reply me-2"></i>@lang('index.reply')
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
